...
#Remove outlier part
plot_boxplot(trainSplit[,c('age','damage_grade')], by = "damage_grade")

trainage<-trainSplit$age
b<-boxplot(trainage)

lowerwhisker<-b$stats[1]
upperwhisker<-b$stats[5]
removed_outlier<-trainSplit[trainSplit$age>lowerwhisker & trainSplit$age<upperwhisker,]

plot_boxplot(removed_outlier[,c('age','damage_grade')], by = "damage_grade")


y_train =  as.data.frame(removed_outlier$damage_grade)
x_train = subset(removed_outlier, select = -c(damage_grade))
...
write.csv(final_x_train,"Removed_Outlier/x_train.csv", row.names=FALSE)
write.csv(final_x_test,"Removed_Outlier/x_test.csv", row.names=FALSE)
write.csv(y_train,"Removed_Outlier/y_train.csv", row.names=FALSE)
write.csv(y_test,"Removed_Outlier/y_test.csv", row.names=FALSE)
...
x_train <- read.csv("Removed_Outlier/x_train.csv")
y_train <- read.csv("Removed_Outlier/y_train.csv")
x_test <- read.csv("Removed_Outlier/x_test.csv")
y_test <- read.csv("Removed_Outlier/y_test.csv")
