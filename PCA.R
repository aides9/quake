...
#PCA part
num_data = train[, c('geo_level_1_id', 'geo_level_2_id', 'geo_level_3_id', 
                     'area_percentage', 'height_percentage',
                     'count_floors_pre_eq', 'age','count_families')]
pca = principal(num_data, nfactors = 1, rotate = "varimax")
print(pca$loadings, cutoff = 0.6)

drops <- c("height_percentage","count_floors_pre_eq")
train = train[ , !(names(train) %in% drops)]
train$PC1 = pca$scores

num_data = test[, c('geo_level_1_id', 'geo_level_2_id', 'geo_level_3_id', 
                     'area_percentage', 'height_percentage',
                     'count_floors_pre_eq', 'age','count_families')]
pca = principal(num_data, nfactors = 1, rotate = "varimax")
print(pca$loadings, cutoff = 0.6)
...
num_data=c("geo_level_1_id", "geo_level_2_id", "geo_level_3_id", "area_percentage", "PC1", "age","count_families")
...
write.csv(final_x_train,"PCA/x_train.csv", row.names=FALSE)
write.csv(final_x_test,"PCA/x_test.csv", row.names=FALSE)
write.csv(y_train,"PCA/y_train.csv", row.names=FALSE)
write.csv(y_test,"PCA/y_test.csv", row.names=FALSE)
...
x_train <- read.csv("PCA/x_train.csv")
y_train <- read.csv("PCA/y_train.csv")
x_test <- read.csv("PCA/x_test.csv")
y_test <- read.csv("PCA/y_test.csv")
